"""Operational phase-space I/O, transforms, and row-selection contracts."""
...
def load_eta_h5(path: str | Path, dataset: str = "eta") -> np.ndarray:
    ...
    return validate_phase_space(eta)
...
def sigma_clip_mask(
    ...
    *,
    ...
    clip_normalizer = fit_normalizer(eta, sample_weight=weights)
...
def load_run_preprocessing(
    ...
    return load_normalizer_npz(normalizer_path), coordinate_transform
...
def save_normalizer_npz(normalizer: Normalizer, path: str | Path) -> None:
    """Persist numerical normalization as an operational artifact."""
    path = Path(path)
    path.parent.mkdir(parents=True, exist_ok=True)
    np.savez(path, mean=normalizer.mean, std=normalizer.std)


def load_normalizer_npz(path: str | Path) -> Normalizer:
    """Restore numerical normalization from an operational artifact."""
