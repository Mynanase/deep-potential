#!/usr/bin/env bash
# Angular structure of the inner-region model density: fluctuation profile,
# multipole spectra, Mollweide maps.
set -eo pipefail
...
echo '=== S1 DENSITY ANGULAR PREFLIGHT ==='
...
"$PY" scripts/auriga/plot_s1_density_angular.py \
  --model base="$BASE" --model s11="$S11" --model S1="$S1" \
  --truth "$TRUTH" --output-dir figures/s1-density-angular

echo '=== OUTPUTS ==='
find figures/s1-density-angular -type f | sort
echo '=== S1 DENSITY ANGULAR DONE ==='
