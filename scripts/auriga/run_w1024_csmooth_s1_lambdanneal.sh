#!/usr/bin/env bash
# Lambda cosine anneal on the negativity penalty (S1 route, w1024,
...
# contract except lambda_eff(t) = 1 + 9*(1+cos(pi*t/T))/2 annealing the
# grid-decoupled negativity penalty from 10 (lambda10 regime) to 1
# (innerA regime) over the 256 epochs. Tests whether lambda10's 30-50 kpc
# over-regularization is a late-training artifact fixable by starting
# strong and relaxing. Logs lambda_eff and prior_neg_grid per epoch.
...
ls_, le_ = float(lo.get("lambda_start", 0.0)), float(lo.get("lambda_end", 0.0))
assert ls_ > le_ > 0.0, "lambda-anneal run needs lambda_start > lambda_end > 0"
assert abs(float(lo.get("lambda_", 0.0)) - le_) < 1e-9, "base lambda_ should equal lambda_end"
...
assert len(lam) == len(p["train"]), "lambda_eff history must cover every epoch"
...
print("lambda_eff per-epoch: first={:.4f} mid={:.4f} final={:.4f}".format(lam[0], lam[len(lam)//2], lam[-1]))
