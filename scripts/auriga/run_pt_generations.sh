...
# cap-w512 3-stage on halo12-clean-smooth-s11 (clean+smooth basis, run 15f446e5)
W512CSM=$HOME/.orx/runs/15f446e5-8358-4857-ab0f-5d63544527c3/repo/runs/orx
...
for d in "$W128RAW" "$W128S1" "$W128CLEAN" "$W512CSM" "$W1024RAW" "$W1024CSM"; do
...
"$PY" scripts/auriga/plot_pt_generations.py \
  ...
  --model w128clean="$W128CLEAN" --model w512csm="$W512CSM" \
  --model w1024raw="$W1024RAW" --model w1024csm="$W1024CSM" \
