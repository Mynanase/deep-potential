#!/usr/bin/env python
"""Radial r-bins (clump-covering) with EACH model judged against its OWN
population truth.

  w1024full, w128   -> truth = halo12-val (full population, clump present;
                       key-921 noise, identical protocol to run 5d657cef)
  w1024clean        -> truth = clean-val (halo12-clean.h5 first 25%;
                       key-922 noise, draws at clean-val positions)

Bin edges (kpc): [0, 10, 20, 30, 45, 64, 75]; [45, 64) contains 100% of the
clump members. Panels overlay both truths (halo12-val filled, clean-val thin
step) with the three model curves; the W1 annotation is per model vs its own
truth. Semantics: each number answers "does this model represent the
population it was trained on" -- cross-model comparison mixes truth sets.

Run from repo root (server):
  CUDA_VISIBLE_DEVICES=6 python scripts/auriga/df_radial_rbin_own.py
"""
...
OUT = REPO / "runs" / "radial-rbin-own-20260917"
...
# two truth populations (each model's own)
truth = {}
for tag, fn in [("h12", "halo12.h5"), ("clean", "halo12-clean.h5")]:
    with h5py.File(REPO / "data" / "auriga" / fn, "r") as f:
        eta_t = f["eta"][:int(f["eta"].shape[0] * 0.25)]
        w_t = f["weights"][:eta_t.shape[0]]
    pos_t = eta_t[:, :3]
    ...
    truth[tag] = dict(eta=eta_t, w=w_t, pos=pos_t, r=r_t,
                      ...
                      v=sph_vel(pos_t, eta_t[:, 3:]), n=len(eta_t))
    print("truth %s: n=%d" % (tag, truth[tag]["n"]), flush=True)

# clump membership (halo12 side) for annotations
...
r_mem = truth["h12"]["r"][mem_h12]
print("clump members in halo12-val: %d ; in [45,64): %d (%.1f%%)"
      % (mem_h12.sum(), ((r_mem >= 4.5) & (r_mem < 6.4)).sum(),
...
for nm, run_dir, tag in MODELS:
    ...
    draws[nm] = sph_vel(np.repeat(tr["pos"], K, axis=0), out).reshape(n, K, 3)
    del flow, cvf
    jax.clear_caches()
...
def panel(ax, b_lo, b_hi, clump_frac=None):
    ...
    mh = (truth["h12"]["r"] >= b_lo) & (truth["h12"]["r"] < b_hi)
    mc = (truth["clean"]["r"] >= b_lo) & (truth["clean"]["r"] < b_hi)
    th = truth["h12"]
    tc = truth["clean"]
    ax.hist(th["v"][mh, ci], bins=bins, weights=th["w"][mh], density=True,
            alpha=0.4, color="C0", label="truth halo12-val (full)")
    ax.hist(tc["v"][mc, ci], bins=bins, weights=tc["w"][mc], density=True,
    ...
    for nm in NAMES:
        ...
        wd = np.repeat(tr["w"][m], K)
        w1p[nm] = w1(tr["v"][m, ci], d, tr["w"][m], wd)
        c, ls, lw = styles[nm]
        ax.hist(d, bins=bins, weights=wd, density=True, histtype="step",
...
edges = R_EDGES
fig, axes = plt.subplots(3, len(edges) - 1, figsize=(19, 7.8), sharex=True)
for ci in range(3):
    for b in range(len(edges) - 1):
        ax = axes[ci, b]
        cf = None
        if b == 4:
            mh = (truth["h12"]["r"] >= edges[b]) & (truth["h12"]["r"] < edges[b + 1])
            cf = float(truth["h12"]["w"][mh & mem_h12].sum() / truth["h12"]["w"][mh].sum())
        panel(ax, edges[b], edges[b + 1], clump_frac=cf)
        ax.set_title("%g-%g kpc" % (edges[b] * 10, edges[b + 1] * 10), fontsize=9)
        if b == 0:
            ax.set_ylabel(COMP[ci], fontsize=10)
axes[0, 0].legend(fontsize=6.5, loc="upper right")
fig.suptitle("each model vs its OWN population truth (bins cover the clump: 45-64 kpc)", fontsize=12)
fig.tight_layout(rect=(0, 0, 1, 0.96))
fig.savefig(OUT / "figs" / "fig_radial_rbin_own_full.png", dpi=150)
plt.close(fig)
...
fig.suptitle("outer bins: per-model own-truth W1 (45-64 kpc holds 100% of clump members)", fontsize=11)
fig.tight_layout(rect=(0, 0, 1, 0.96))
fig.savefig(OUT / "figs" / "fig_radial_rbin_own_outer.png", dpi=150)
plt.close(fig)
