...
Inner-band-fix pt-adjudication: five w1024 clean+smooth Phis - base
(production co-winner), S1 (gridprior parent), gridprior (run 09faad30,
volume grid), innerA (run 2b32eb04, radius-balanced grid) and innerB
(run 7016399a, inner-weighted volume grid) - against the particle-monopole
truth of the grid product. Decision metric: inner-band dM (2-10, 10-30 kpc).
...
def main():
    ...
    ap.add_argument("--output-dir", type=Path,
                    default=Path("figures/pt-adjudication-innerband"))
    ...
    np.savez(args.output_dir / "pt_adjudication_innerband.npz",
             ...
             **{f"{l}_{k}": v for l in MODELS
                for k, v in (("m_flux", m_flux[l]), ("rel", rel_node[l]),
                             ("frac_neg", frac_neg[l]),
                             ("dm_flux", dm_fluxes[l]))})
    for p in ofs.save(fig, str(args.output_dir / "pt-adjudication-innerband")):
        print("saved", p)
