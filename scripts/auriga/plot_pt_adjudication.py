...
Methods transplanted from the frozen adjudication line, with the truth side
now the particle monopole from the grid product:
...
# Published 60-shell adjudication, run 0454cb6b @86a3ef7 (node 532eada5),
# for side-by-side comparison only - not used in any computation here.
REF_60SHELL = {"base": (2.34, 6.64, 9.11, 7.46),
               "s11": (2.55, 6.89, 10.98, 10.46),
...
def main():
    ...
    ap.add_argument("--output-dir", type=Path,
                    default=Path("figures/pt-adjudication"))
    ...
    print("=== median |rel err| of FLUX dM per radial band [%] "
          "(this run / published 60-shell run 0454cb6b) ===")
    band_med = {}
    ...
    for l in MODELS:
        ...
        print(f"  {l}: frac(rho<=0) {vals.min()*100:.1f}-{vals.max()*100:.1f}% "
              ...
              "were 16.1/18.1/15.2% (node d69144c0)")
