...
Lambda-sweep pt-adjudication: six w1024 clean+smooth Phis - base
(production co-winner), S1 (gridprior parent), volume-grid gridprior
(run 09faad30), innerA lambda=1 (run 2b32eb04, radius-balanced grid,
dM 1.25/5.95/8.76/8.14), lambda=0.1 (run f6c902f6, negativity collapsed
to ~12%) and lambda=10 (run e1cf8f74, negativity ~0.8%) - against the
particle-monopole truth of the grid product. Decision: does lambda=10's
near-zero negativity come at a dM distortion cost vs lambda=1?
...
COLOR = {"base": "blue", "S1": "red", "gridprior": "green",
         "innerA": "purple", "lambda0p1": "cyan", "lambda10": "orange"}
...
def main():
    ...
    ap.add_argument("--output-dir", type=Path,
                    default=Path("figures/pt-adjudication-lambda"))
    ...
    for bi, (lo, hi) in enumerate(BANDS):
        ...
        for l in MODELS:
            ...
            ref = (f"{REF_60SHELL[l][bi]:5.2f}" if l in REF_60SHELL
                   else "    -")
            row += f"{l} {med:5.2f}/{ref}   "
        print(row)
    ...
    for l in MODELS:
        ...
        print(f"  {l}: frac(rho<=0) {vals.min()*100:.1f}-{vals.max()*100:.1f}% "
              f"(mean {vals.mean()*100:.1f}%)")
    print("  anchor note: whole-domain midplane cells were 17.0/15.6/3.6% "
          "base/S1/gridprior (node 658d75bd)")
    ...
    np.savez(args.output_dir / "pt_adjudication_lambda.npz",
             ...
             **{f"{l}_{k}": v for l in MODELS
                for k, v in (("m_flux", m_flux[l]), ("rel", rel_node[l]),
                             ("frac_neg", frac_neg[l]),
                             ("dm_flux", dm_fluxes[l]))})
    for p in ofs.save(fig, str(args.output_dir / "pt-adjudication-lambda")):
        print("saved", p)
