#!/usr/bin/env bash
# Osc-pair smoothness penalty on top of inner-band fix A (radius-balanced
# grid-decoupled negative-density prior, S1 route, w1024 clean+smooth).
# Child of the frozen innerA winner (9788e6de / run 2b32eb04): identical
# contract except the Phi loss adds an oscillation penalty. Every epoch the
# prior grid points q_g are paired with q_g + delta*u (delta ~ U(2,6) kpc,
# u a random unit vector) and the loss gains
#     osc_weight * mean_grid[(Lap(Phi)(q_pair) - Lap(Phi)(q_grid))^2]
# (dimensionless Laplacian units, outside the CBE log, like prior_neg):
# a Dirichlet-energy penalty on high-frequency density changes at 2-6 kpc
# scales. osc_weight is calibrated by the density-spectrum diagnostic
# (node 2484e3ab) so the term contributes O(1) at innerA oscillation level.
# Prior grid decoupling and S1 sampling stay as in innerA.
...
assert float(lo.get("osc_weight", 0.0)) > 0.0, "osc-pair run must set osc_weight > 0"
assert 0.0 < float(lo.get("osc_delta_min_kpc", 0.0)) < float(lo.get("osc_delta_max_kpc", 0.0)), "osc-pair run needs 0 < osc_delta_min_kpc < osc_delta_max_kpc"
...
osc = p.get("osc_pen_grid", [])
assert len(osc) == len(p["train"]), "osc_pen_grid history must cover every epoch"
...
print("osc_pen_grid per-epoch mean squared Laplacian difference: first={:.3e} mid={:.3e} final={:.3e} min={:.3e}".format(osc[0], osc[len(osc)//2], osc[-1], min(osc)))
