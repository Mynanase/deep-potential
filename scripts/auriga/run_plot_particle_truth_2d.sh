...
"$PY" scripts/auriga/plot_particle_truth_2d.py \
  --grids data/auriga/halo12_particle_truth_grids.h5 \
  --model base="$BASE" --model s11="$S11" --model S1="$S1" \
  "${TRUTH_ARG[@]}" --output-dir figures/particle-truth-2d

echo '=== OUTPUTS ==='
find figures/particle-truth-2d -type f | sort
echo '=== PT FIGURES DONE ==='
