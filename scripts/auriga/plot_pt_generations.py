...
log-log, (b) signed rel err with a +/-5% band. Known labels (base,
lambda10, innerA, gridprior) keep their cross-figure colors; unknown
labels fall back to the palette cycle.
"""
import argparse
import sys
import time
...
COLOR = {"base": "blue", "lambda10": "orange", "innerA": "purple",
         "gridprior": "green"}
...
def main():
    ...
    for i, l in enumerate(labels):
        ax.plot(r_nodes[1:], dm_fluxes[l][1:],
                color=ofs.PALETTE[COLOR.get(l, palette[i % 6])],
    ...
    for i, l in enumerate(labels):
        ax.plot(r_nodes[1:], 100.0 * rels[l][1:],
                color=ofs.PALETTE[COLOR.get(l, palette[i % 6])], lw=1.2, marker="o",
                ms=2.5, label=l)
