#!/usr/bin/env bash
# Inner-band fix B: inner-weighted grid-decoupled negative-density prior on
# the S1 route (w1024, clean+smooth). Child of the frozen gridprior winner
# (863ef6a2 / run 09faad30): identical contract except the prior grid weights.
...
# independent spatial grid. The frozen parent used the plain volume-weighted
# mean over that grid (only ~7.9% of points inside 30 kpc; probe inner
# negativity 11.5%); this variant keeps the grid volume-weighted everywhere
# (outer prior identical to the parent by construction) and upweights the
# penalty mean inside 30 kpc by a factor 8 (effective inner share ~40.6%):
#     loss = log(sum w*cbe_i/sum w) + lambda * mean_grid(prior_neg)
# with mean_grid = sum(w_grid prior_neg)/sum(w_grid), w_grid = 8 for r<30 kpc
# else 1. q_grid is a batch channel in get_phi_loss; train_potential resamples
...
assert float(lo.get("prior_grid_inner_weight", 1.0)) == 8.0, \
    "inner-band fix B must set prior_grid_inner_weight=8"
assert float(lo.get("prior_grid_inner_r_kpc", 0.0)) == 30.0, \
    "inner-band fix B must set prior_grid_inner_r_kpc=30"
assert "prior_grid_weighting" not in lo, \
    "inner-band fix B keeps the volume sampler (no prior_grid_weighting)"
...
out = dict(mode="inner_weighted_volume_grid", band=band, uniform_ball_probe=probe)
