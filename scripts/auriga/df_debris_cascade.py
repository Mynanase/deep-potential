#!/usr/bin/env python
"""55-65 kpc removal cascade: what each removal stage takes out.

Compares, in one radial bin (55-65 kpc), the spherical velocity components
of four nested populations of the Halo12 stellar sample:

  stage 0: raw halo12 (no removal),
  stage 1: union-removed population (clump detection + smooth84 registry),
  stage 1.5: stage 1 plus the 129 v1 6D-FOF debris candidates removed
             (clump_debris_candidates.npz; disjoint from detection/smooth
             registries by construction),
  stage 2: stage 1.5 plus the 638 v2 velocity-gate debris candidates
           removed (run e94c3979, clump_debris_candidates_v2.npz; overlaps
           v1 by 51 pids, so the two removal layers are not nested).
...
and decomposed into what stage 1 removed, what the v1 FOF removed, what the
v2 gate removed, and what survives both removals, so "no removal" and
"after all removals" are directly comparable bin by bin. A second zoom
figure shows only the v1 and v2 removal layers at their own mass scale.
...
v1_pids = np.sort(np.load(V1_NPZ)["debris_pids"].astype(np.int64))

print("=== DEBRIS CASCADE 55-65 kpc ===", flush=True)
print(f"raw n={len(pid_raw)}, union-kept n={len(pid_union)}, "
      f"v1 candidates n={len(v1_pids)}, v2 candidates n={len(debris_pids)}",
      flush=True)
...
v1_in_raw = np.isin(v1_pids, pid_raw)
print(f"v1 pids found in raw: {int(v1_in_raw.sum())}/{len(v1_pids)}; "
      f"in union-kept (survive stage 1): {int(np.isin(v1_pids, pid_union).sum())}; "
      f"also in v2: {int(np.isin(v1_pids, debris_pids).sum())}", flush=True)
assert v1_in_raw.all(), "v1 pids missing from raw export"
...
for lo, hi in CTX_BINS:
    m0, m1, m2, m3 = stage_masks(lo, hi)
    s0, s1, s2, s3 = stats(m0), stats(m1), stats(m2), stats(m3)
    rem1 = s0["mass_1e6_msun"] - s1["mass_1e6_msun"]
    remv1 = s1["mass_1e6_msun"] - s2["mass_1e6_msun"]
    rem2 = s2["mass_1e6_msun"] - s3["mass_1e6_msun"]
    print(f"bin {lo:.0f}-{hi:.0f} kpc: raw n={s0['n']} m={s0['mass_1e6_msun']:.2f} | "
          f"stage1 removed m={rem1:.2f} ({100 * rem1 / s0['mass_1e6_msun']:.1f}%) | "
          f"v1 removed m={remv1:.2f} | "
          f"stage2 removed m={rem2:.2f} ({100 * rem2 / s0['mass_1e6_msun']:.1f}% of raw) | "
          f"kept n={s3['n']} m={s3['mass_1e6_msun']:.2f}", flush=True)
    print(f"   frac(vT<-100 km/s): raw={s0['frac_vT_lt_minus100']:.3f} "
          f"after1={s1['frac_vT_lt_minus100']:.3f} after-v1={s2['frac_vT_lt_minus100']:.3f} "
          f"after2={s3['frac_vT_lt_minus100']:.3f} | "
          f"mean vT: raw={s0['vT_mean_kms']:+.1f} after1={s1['vT_mean_kms']:+.1f} "
          f"after-v1={s2['vT_mean_kms']:+.1f} after2={s3['vT_mean_kms']:+.1f} km/s",
          flush=True)
    for key, _ in COMP:
        w1_03 = wasserstein_distance(v[key][m0], v[key][m3], mass[m0], mass[m3])
        w1_01 = wasserstein_distance(v[key][m0], v[key][m1], mass[m0], mass[m1])
        w1_12 = wasserstein_distance(v[key][m1], v[key][m2], mass[m1], mass[m2])
        w1_23 = wasserstein_distance(v[key][m2], v[key][m3], mass[m2], mass[m3])
        rows.append(dict(bin_lo_kpc=lo, bin_hi_kpc=hi, comp=key,
                         w1_raw_vs_after1_kms=w1_01,
                         w1_after1_vs_afterv1_kms=w1_12,
                         w1_afterv1_vs_after2_kms=w1_23,
                         w1_raw_vs_after2_kms=w1_03))
        if (lo, hi) == (R_LO, R_HI):
            print(f"   W1({key}): raw->after1 {w1_01:.1f}, after1->after-v1 {w1_12:.1f}, "
                  f"after-v1->after2 {w1_23:.1f}, raw->after2 {w1_03:.1f} km/s",
                  flush=True)
...
for lo, hi in CTX_BINS:
    m0, m1, m2, m3 = stage_masks(lo, hi)
    for stage, m in (("raw", m0), ("after1", m1), ("after_v1", m2), ("after2", m3)):
        stat_rows.append(dict(bin_lo_kpc=lo, bin_hi_kpc=hi, stage=stage, **stats(m)))

# ---- figure 1: raw histogram decomposed into all removal layers + kept ----
use_style()
fig, axes = figure_grid(1, 3, width=WIDE, ratio=0.36, sharey=True)
m0, m1, m2, m3 = stage_masks(R_LO, R_HI)
m_rem1 = m0 & ~m1          # stage 1: detection + smooth84
m_remv1 = m1 & ~m2         # v1: 6D FOF candidates
m_rem2 = m2 & ~m3          # stage 2: v2 velocity gate
...
for ax, (key, label) in zip(axes, COMP):
    ...
    ax.bar(centers, kept, width=np.diff(V_EDGES), color=PALETTE["blue"],
           edgecolor="none", label="after all removals", zorder=3)
    ax.bar(centers, cut2, width=np.diff(V_EDGES), bottom=kept, color=PALETTE["orange"],
           edgecolor="none", label="removed by v2 velocity gate", zorder=3)
    ax.bar(centers, cutv1, width=np.diff(V_EDGES), bottom=kept + cut2,
           color=PALETTE["red"], edgecolor="none", label="removed by v1 6D FOF", zorder=3)
    ax.bar(centers, cut1, width=np.diff(V_EDGES), bottom=kept + cut2 + cutv1, color=MUTED,
    ...
    ax.set_xlim(-450, 450)
    ax.set_ylim(0, 1.06 * ymax)   # explicit shared limit; fixes clipped peaks
...
try:
    fig.legend([handles[i] for i in order], [labels[i] for i in order],
               loc="outside lower center", ncols=2, fontsize=6.5)
...
save(fig, str(OUT / "figs" / "fig_debris_cascade_55_65"), formats=("png", "pdf", "svg"))

# ---- figure 2: zoom on the v1 and v2 removal layers only ----
fig2, axes2 = figure_grid(1, 3, width=WIDE, ratio=0.36, sharey=True)
ymax2 = 0.0
for ax, (key, label) in zip(axes2, COMP):
    def hist(mask):
        h, _ = np.histogram(v[key][mask], bins=V_EDGES, weights=mass[mask])
        return h / 1e6
    ...
    cutv1 = hist(m_remv1)
    both = cut2 + cutv1
    ymax2 = max(ymax2, float(both.max()))
    ax.bar(centers, cut2, width=np.diff(V_EDGES), color=PALETTE["orange"],
           edgecolor="none", label="removed by v2 velocity gate", zorder=3)
    ax.bar(centers, cutv1, width=np.diff(V_EDGES), bottom=cut2,
           color=PALETTE["red"], edgecolor="none", label="removed by v1 6D FOF", zorder=3)
    ax.step(V_EDGES, np.append(both, both[-1]), where="post", color="black",
            lw=1.0, label="all debris removed", zorder=4)
    ax.axvline(template[key], color=BASELINE, lw=0.8, ls=(0, (2, 2)), zorder=2,
               label="stream-core template (gate center)")
    ax.set_xlabel(label + " (km/s)")
    ax.set_xlim(-450, 450)
    ax.set_ylim(0, 1.06 * ymax2)
axes2[0].set_ylabel(r"mass per bin ($10^6\,M_\odot$)")
panel_labels(axes2)
handles2, labels2 = axes2[0].get_legend_handles_labels()
order2 = [2, 1, 0, 3]
try:
    fig2.legend([handles2[i] for i in order2], [labels2[i] for i in order2],
                loc="outside lower center", ncols=2, fontsize=6.5)
except (TypeError, ValueError):
    axes2[0].legend([handles2[i] for i in order2], [labels2[i] for i in order2],
                    fontsize=5.5, loc="upper left")
save(fig2, str(OUT / "figs" / "fig_debris_cascade_55_65_zoom"),
     formats=("png", "pdf", "svg"))
...
with open(OUT / "summary.json", "w") as fj:
    json.dump(dict(bins_kpc=CTX_BINS, focus_bin_kpc=[R_LO, R_HI],
                   ...
                   v1=dict(file=str(V1_NPZ), n=int(len(v1_pids)),
                           n_in_v2=int(np.isin(v1_pids, debris_pids).sum())),
