#!/usr/bin/env bash
# w1024clean vs clean truth v2: clump sample by pid registry.
set -eo pipefail
...
echo '=== CLEAN-TRUTH PID PREFLIGHT ==='
test -x "$PY" || { echo "PREFLIGHT FAIL: missing $PY"; exit 2; }
test -f "$REG" || { echo "PREFLIGHT FAIL: missing $REG"; exit 2; }
test -f "$AUDIT/eval_protocol.npz" || { echo "PREFLIGHT FAIL: missing audit protocol"; exit 2; }
test -f "$W1024FULL/models/df/flow/flow-21_model.eqx" || { echo "PREFLIGHT FAIL: missing w1024full checkpoint"; exit 2; }
...
"$PY" -u scripts/auriga/df_clean_truth_pid.py
echo '=== CLEAN-TRUTH PID DONE ==='
