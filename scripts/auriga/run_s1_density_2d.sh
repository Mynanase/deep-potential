#!/usr/bin/env bash
# Log-color 2D density maps, truth comparison, and residuals for the three
# adjudicated w1024 clean+smooth Phis.
set -eo pipefail
...
"$PY" scripts/auriga/plot_s1_density_2d.py \
  ...
  --truth "$TRUTH" --output-dir figures/s1-density-2d

echo '=== OUTPUTS ==='
find figures/s1-density-2d -type f | sort
echo '=== S1 DENSITY 2D DONE ==='
