#!/usr/bin/env bash
# Spectral-norm ceiling on the potential MLP (S1 route, w1024,
# clean+smooth, radius-balanced grid-decoupled negative-density prior).
# Child of the frozen innerA winner (9788e6de / run 2b32eb04): identical
# contract except that after every epoch each Linear spectral norm of the
# phi MLP is clipped back to its initialization value sigma_0:
# W <- W * min(1, sigma_0/sigma(W)) (exact SVD). This bounds the Lipschitz
# constant of Phi at its initialization scale (Miyato et al. 2018
# mechanism), blocking progressive high-frequency sharpening while
# preserving init capacity. Logs sn_sigma_ratio_max / sn_clipped_frac.
...
assert bool(lo.get("sn_ceiling", False)) is True, "sn-ceiling run must set sn_ceiling=true"
...
sn = p.get("sn_sigma_ratio_max", [])
cl = p.get("sn_clipped_frac", [])
assert len(sn) == len(p["train"]), "sn_sigma_ratio_max history must cover every epoch"
assert len(cl) == len(sn), "sn_clipped_frac history must match"
...
print("sn_sigma_ratio_max per-epoch: first={:.3f} mid={:.3f} final={:.3f} max={:.3f}".format(sn[0], sn[len(sn)//2], sn[-1], max(sn)))
print("sn_clipped_frac per-epoch: first={:.1%} mid={:.1%} final={:.1%} max={:.1%}".format(cl[0], cl[len(cl)//2], cl[-1], max(cl)))
