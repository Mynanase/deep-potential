# Archived TensorFlow/Sonnet implementation

> 本目录仅用于保存项目早期的 TensorFlow、TensorFlow Probability 和
> Sonnet 实现，不属于当前 JAX 训练、评估或打包流程。其依赖没有在
> `pyproject.toml` 中声明，代码也不再维护；如需复现实验，应使用与历史
> commit 匹配的独立环境。

# Deep Potential (Legacy TensorFlow) 脚本结构说明文档

> ⚠️ **重要警示**：
> 本归档目录 (`archive/legacy_tensorflow/scripts/`) 包含了项目早期基于
> **TensorFlow** 和 **Sonnet** 框架构建的代码实现。
> 这些代码目前已被 `dpjax/` 中的 **JAX/Flax** 重构版所取代。文档中所描述的 `tf.Variable`、`snt.Module` 等模型构建方法在最新的开发与主分支中**已被完全废除**，请勿在当前业务流中使用。
> 编写本文档的目的仅用于历史对照。仍被当前流程需要的 Plummer 采样逻辑已
> 提取到 `dpjax/datasets/plummer.py`，不依赖本归档。
...
Plummer 数据生成器仍是当前流程的重要资产，但已迁移到活动目录。以下说明
对应 `experiments/gendata_plummer.py` 与
`dpjax/datasets/plummer.py`，而不是本归档中的可执行入口：
